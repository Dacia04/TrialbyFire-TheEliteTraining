...
public class GolemLaserState : GolemState
{
    ...
    public override void Update()
    {
        ...
        {
            if(Mathf.Abs(Golem.transform.position.y - Golem.Pos1Idle.position.y)  > 0.05f  && Golem.transform.position.y - Golem.Pos1Idle.position.y>0)
                Golem.Rigidbody2D.velocity = new Vector2(GetGolemVelovity().x,Golem.EnemySO.GolemData.SpeedIdle * -1f);
            else
            {
